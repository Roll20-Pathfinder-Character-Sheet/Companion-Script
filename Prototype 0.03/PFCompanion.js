...
var PFCompanion = PFCompanion || (function() {
    ...
    checkInstall = function() {
        //state.PFCompanion.TAS = 'auto' //Uncomment to activate auto token setup
        //state.PFCompanion.ResourceTrack = true; //uncomment to activate resource tracking
    ...
    initialize = async function(character){
        var characters;
    ...
    initializeRepeatingResourceTracking = function(r,attributes){
        ...
        macroTextName = _.filter(attributes,(a)=>{
            return (a.get('name').toLowerCase().indexOf(r.toLowerCase()+'_name')>-1);
        });
        macroTextName = macroTextName.length>0 ? macroTextName[0].get('name').replace('name',((isNPC && macroTextName.indexOf('item')===-1) ? 'NPC-macro-text' : 'macro-text')) : undefined;
        macroText = macroTextName ? getAttrByName(attributes[0].get('characterid'),macroTextName) : undefined;
        macroText = macroText!=='' ? macroText : undefined;
        macroTextObject = macroText ? _.filter(attributes,(a)=>{return a.get('name')===macroTextName})[0] : createObj('attribute',{
        ...
        sourceSpell = _.filter(attributes,(a)=>{
            return a.get('name').toLowerCase().indexOf(r.toLowerCase()+'_source-spell')>-1;
        });
        sourceAbility = _.filter(attributes,(a)=>{
            return a.get('name').toLowerCase().indexOf(r.toLowerCase()+'_source-ability')>-1;
        });
        usesAmmo = _.filter(attributes,(a)=>{
            return a.get('name').toLowerCase().indexOf(r.toLowerCase()+'_ammo')>-1;
        });
        sourceSpell = sourceSpell.length>0 ? sourceSpell[0] : undefined;
        sourceAbility = sourceAbility.length>0 ? sourceAbility[0] : undefined;
        usesAmmo = usesAmmo.length>0 ? (usesAmmo[0].get('current')==='0' ? false : true) : false;
        rollTemplate = macroTextObject ? (macroTextObject.get('current').match(/(?:&{template:([^}]+)})/) ? macroTextObject.get('current').match(/(?:&{template:([^}]+)})/)[1] : undefined)  : undefined;
        if(macroTextObject){
            toAdd+= ((!macroTextObject.get('current').match(/\|\|rowid=[^\|]+\|\|/)) ? '||rowid='+r+'||' : '')
                +((!macroTextObject.get('current').match(/\|\|spellid=@{source-spell}\|\|/) && sourceSpell) ? '||spellid=@{source-spell}||' : '')
                +((!macroTextObject.get('current').match(/\|\|abilityid=@{source-ability}\|\|/) && sourceAbility) ? '||abilityid=@{source-ability}||' : '')
                +((!macroTextObject.get('current').match(/\|\|ammo=[^\|]+\|\|/) && usesAmmo) ? '||ammo=?{Name of Ammunition Item}||' : '');
            (toAdd.length>2 && rollTemplate) ? macroTextObject.set('current',[macroTextObject.get('current').slice(0,macroTextObject.get('current').indexOf(rollTemplate)+rollTemplate.length+ 1),' '+toAdd,macroTextObject.get('current').slice(macroTextObject.get('current').indexOf(rollTemplate)+rollTemplate.length+ 1)].join('')) : undefined;
        }
    },
    
    deleteAmmoTracking = function(r,attributes){
        var macroTextName = _.filter(attributes,(a)=>{return a.get('name').indexOf(r+'_name')>0}),
            isNPC = getAttrByName(attributes[0].get('characterid'),'is_npc')==='0' ? false : true,
            macroTextObject,ammoString;
            
        macroTextName = macroTextName ? macroTextName[0].get('name').replace('name',(macroTextName[0].get('name').indexOf('item')=== -1 && isNPC ? 'NPC-macro-text' : 'macro-text')) : undefined;
        macroTextObject = macroTextName ? _.filter(attributes,(a)=>{return a.get('name')===macroTextName}) : undefined;
        macroTextObject = macroTextObject ? macroTextObject[0] : undefined;
    ...
    handleAmmo = function(ammo,attributes,character,msg,rollId){
        var ammoId,ammoCount,ammoUsed,macroText,ammoQuery,
        ...
        if(ammoId){
            ammoCount = _.filter(attributes,(a)=>{return a.get('name')===('repeating_item_'+ammoId+'_qty')})[0];
            ammoUsed = msg.content.match(/attack\d*=/g).length;
            sendChat('Ammo Tracking','@{'+character.get('name')+'|'+(getAttrByName(character.id,'is_npc')===0 ? 'PC-whisper':'NPC-whisper')+'}&{template:pf_block} @{'+character.get('name')+'|toggle_accessible_flag} @{'+character.get('name')+'|toggle_rounded_flag} {{color=@{'+character.get('name')+'|rolltemplate_color}}} {{header_image=@{'+character.get('name')+'|header_image-pf_block-item}}}'
            +'{{character_name=@{'+character.get('name')+'|character_name}}} {{character_id=@{'+character.get('name')+'|character_id}}} {{subtitle}} {{name=@{'+character.get('name')+'|repeating_item_'+ammoId+'_name}}} {{hasuses=@{'+character.get('name')+'|repeating_item_'+ammoId+'_has_uses}}} {{=-'+ammoUsed+'}} {{qty=@{'+character.get('name')+'|repeating_item_'+ammoId+'_qty}}} {{qty_max=@{'+character.get('name')+'|repeating_item_'+ammoId+'_qty|max}}}'
            ...
            macroText = _.filter(attributes,(a)=>{
            ...
            macroText = macroText.length>0 ? macroText[0] : undefined;
