...
var PFCompanion = PFCompanion || (function() {
    ...
    var version = 'Prototype 0.06',
        lastUpdate = 1493868185,
        schemaVersion = 0.06,
    ...
    generateHelp = function(){
        ...
        helpCharacter = helpCharacter ? helpCharacter : createObj('character',{name:'Pathfinder Companion',archived:true,inplayerjournals:'all'});
        state.PFCompanion.helpLink = state.PFCompanion.helpLink ? state.PFCompanion.helpLink : helpCharacter.id;
    ...
    initializeWeapon = function(character,macroTextObject,attributes,isNPC,rowID){
        ...
        toAdd += rollTemplate ? ((!macroText.match(/\|\|rowid=.+\|\|/) ? '||rowid='+rowID+'||' : '')
                ...
                +((!macroText.match(/\|\|item=.+\|\|/) && usesAmmo) ? '||item=?{Name of Ammunition Item}||' : '')) : '';
    ...
    initializeAbility = function(character,macroTextObject,attributes,isNPC,rowID){
        ...
        duplicate = duplicate ? _.reject(duplicate,(d)=>{return d.indexOf(abilityName.get('current'))>-1}) : undefined;
        _.each(duplicate,(d)=>{
    ...
    deleteAmmoTracking = function(r,attributes){
        ...
        ammoString = macroTextObject ? macroTextObject.get('current').match(/\|\|item=([^\|]+)\|\|/) : undefined;
