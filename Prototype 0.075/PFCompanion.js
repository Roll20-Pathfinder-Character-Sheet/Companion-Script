...
var PFCompanion = PFCompanion || (function() {
    ...
    parseStatblock = function(statblock,source){
    ...
    statblockHandler = async function(text,source,characters){
        var block,iChar,isnpc,blockSection,attributes,
            convert = {
                ...
                'statistics':[null,'str_compendium','dex_compendium','con_compendium','int_compendium','wis_compendium','cha_compendium','bab_compendium','cmd_compendium','npc-feats-text','skills_compendium','languages','SQ_compendium'],
                'tactics':[null,'tactics'],
                'ecology':[null,'ecology'],
                'special':[null,'content_compendium'],
                'default':[null,'avatar','token','description','character_name','cr_compendium','xp_compendium','class','alignment','size_compendium','type_compendium','subtype','init_compendium','sense_compendium','weaknesses','npc-aura']
            },
            accrue={},
            section,keys,
            start = _.now();
            
        text =_.map(text.replace(/<br\/>/g,'').replace(/<br>/g,'\n').split(/\nNEW CREATURE\n/),(t)=>{
        ...
        for(var i=0;i<text.length;i++){
            if(!_.isEmpty(text[i])){
                if(characters){
                    ...
                    iChar=createObj('character',{name:i+''});
                }
                attributes=findObjs({type:'attribute',characterid:iChar.id});
                while(blockSection = text[i].shift()){
                    block = await parseStatblock(blockSection,source);
                    if(block){
                        _.each(block,(b)=>{
                ...
                setParseAttr(accrue,iChar,attributes);
                    //log('  > Pathfinder Companion Statblock Parser:'+keys[a]+' parsed <');
                await createAttrWithWorker('npc_import_now',iChar.id,attributes,1);
                log('  > Pathfinder Companion Statblock Parser:'+iChar.get('name')+' imported <');
            }
        }
        log('  > Pathfinder Companion Statblock Parser:'+text.length+' NPCs parsed and imported in '+((_.now()-start)/1000)+' seconds <');
    },
    
    setParseAttr = function(accrue,iChar,attributes){
        var keys = _.keys(accrue),
            setAttr;
        _.each(keys,(k)=>{
            if(accrue[k]){
                switch(true){
                    ...
                    if(cmdDetails.details.source){
                        ...
                        if(characters){
                            ...
                            Promise.all(charText).then((t)=>{
                                ...
                                statblockHandler(chatText.join('\nNEW CREATURE\n'),cmdDetails.details.source,characters);
                            });
                        }else{
                            statblockHandler(chatText.join('\nNEW CREATURE\n'),cmdDetails.details.source);
                        }
                    }
                    break;
                case 'apply':
	...
	cmdExtract = function(cmd){
        ...
        _.each(details,(d)=>{
            vars=d.match(/(limit|ignore|item|spell|class|ability|note|current|max|pc|npc|stats|buff|condition|hp|tracking|TAS|source)(?:\:|=)([^,]+)/) || null;
            if(vars){
