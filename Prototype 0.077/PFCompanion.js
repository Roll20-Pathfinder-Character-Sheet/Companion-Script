...
var PFCompanion = PFCompanion || (function() {
    ...
    var version = 'Prototype 0.077',
        sheetVersion = 1.5,
        lastUpdate = 1494302806,
        schemaVersion = 0.077,
        defaults = {
            css: {
                button: {
    ...
    statblockHandler = async function(text,characters){
        ...
        var block,iChar,blockSection,attributes,
            convert = {
                ...
                'statistics':[null,'str_compendium','dex_compendium','con_compendium','int_compendium','wis_compendium','cha_compendium','bab_compendium','cmb_compendium','cmd_compendium','npc-feats-text','skills_compendium','racial_mods_compendium','languages','SQ_compendium'],
                    ...
                    _.each(keys,(k)=>{
                        switch(true){
                            ...
                            case k==='tactics':
                                ...
                                break;
                            case k==='spells':
                                accrue[k]=accrue[k].replace(/\nSpell-Like Abilities\s+/i,'___Spell Parse Site___\nSpell-Like Abilities ').replace(/\nSpells Prepareds\s+/i,'___Spell Parse Site___\nSpells Prepared ').replace(/\nSpells Known\s+/i,'___Spell Parse Site___\nSpells Known ').replace(/\nPsychic Magic\s+/i,'___Spell Parse Site___\nPsychic Magic ').replace(/\nPsychic Spells Known\s+/i,'___Spell Parse Site___\nPsychic Spells Known ').split('___Spell Parse Site___');
                                _.each(accrue[k],(a)=>{
                                    if(a.match(/^(?:\n)?(?:Spell-Like Abilities|Psychic Magic)\s/)){
                            ...
                            case k==='cr_compendium':
                                if(accrue[k].match(/\/MR\s+\d+/)){
                                    ...
                                    if(ratings){
                                        ...
                                        createAttrWithWorker('mythic-adventures-show',iChar.id,attributes,'1');
                                    }
                                }
                                break;
                            default:
