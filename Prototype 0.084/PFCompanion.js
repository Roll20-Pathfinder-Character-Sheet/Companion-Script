...
var PFCompanion = PFCompanion || (function() {
    ...
    makeStatusButton = function(command,status,condition) {
    ...
    applyConditions = async function(character,condition,buff,swap,remove,rounds){
        ...
        if(rounds || remove){
            ...
            if(turnTracker){
                ...
                if(buffNameAttr){
                    buffTrackName = character.get('name')+' Buff: '+buffNameAttr.get('current');
                    _.some(_.range(turnOrder.length),(r)=>{
	...
	cmdExtract = function(cmd){
        ...
        cmdSep.action = details.match(/config|help|TAS|resource|rest|whisper|apply|parse|grouproll/);
        if(cmdSep.action){
