...
var PFCompanion = PFCompanion || (function() {
    ...
    applyConditions = async function(character,condition,buff,swap,remove,rounds){
        ...
        var attributes = findObjs({type:'attribute',characterid:character.id}),
            ...
            conditionNameArr = ['Fatigued','Exhausted','Blinded','Entangled','Invisible','Cowering','Fear','Pinned','Dazzled','Flat-Footed','Prone','Deafened','Grappled','Sickened','Helpless','Stunned','Energy Drain'],
    ...
    applyStatus = function(character,status,state){
        var graphic = findObjs({type:'graphic',represents:character.id})[0];
        if(!graphic){return}
    ...
    tokenHandler = function(who,token,details){
        ...
        if(!_.isEmpty(detailKeys)){
            ...
            msg+='<b>Represents</b><div style="float:right">'+ makeButton('!pfc --token,represents=?{Name of Character} --token',(_.isEmpty(token.get('represents')) ? 'NONE' : getObj('character',token.get('represents')).get('name')),'transparent','black') +'</div><div style="clear: both"></div>'
            ...
            +'<b>Width</b><div style="float:right;">'+makeButton('!pfc --token,width=?{Unit Width} --token',token.get('width'),'transparent','black')+'</div><div style="clear:both;">'
            +'<b>Height</b><div style="float:right;">'+makeButton('!pfc --token,height=?{Unit Height} --token',token.get('height'),'transparent','black')+'</div><div style="clear:both;">'
            //Bar Settings
            +'<br><div style="textalign:center;font-size:110%;font-weight:bold;">Bars</div>'
            +'<table style="width:100%;table-layout:fixed;overflow:hidden;word-wrap:break-all;">'
                +'<colgroup>'
                ...
                +'</tr><tr>'
                    +'<td><b>Link</b></td>'
                    +'<td>'+makeButton('!pfc --token,bar3_link='+linkQuery+' --token',links.bar3 ? links.bar3.get('name') : '_','transparent',links.bar3 ? 'black' : 'transparent')+'</td>'
                    +'<td>'+makeButton('!pfc --token,bar1_link='+linkQuery+' --token',links.bar1 ? links.bar1.get('name') : '_','transparent',links.bar1 ? 'black' : 'transparent')+'</td>'
                    +'<td>'+makeButton('!pfc --token,bar2_link='+linkQuery+' --token',links.bar2 ? links.bar2.get('name') : '_','transparent',links.bar2 ? 'black' : 'transparent')+'</td>'
                +'</tr><tr>'
                    +'<td><b>Value</b></td>'
                    +'<td>'+makeButton('!pfc --token,bar3_value=?{Pull Values From} --token',(token.get('bar3_value')+'/'+token.get('bar3_max')),'transparent','black')+'</td>'
                    +'<td>'+makeButton('!pfc --token,bar1_value=?{Pull Values From} --token',(token.get('bar1_value')+'/'+token.get('bar1_max')),'transparent','black')+'</td>'
                    +'<td>'+makeButton('!pfc --token,bar2_value=?{Pull Values From} --token',(token.get('bar2_value')+'/'+token.get('bar2_max')),'transparent','black')+'</td>'
                +'</tr><tr>'
                    +'<td><b>Visible</b></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,showplayers_bar3='+(token.get('showplayers_bar3') ? 'off' : 'on')+' --token',(token.get('showplayers_bar3') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,showplayers_bar1='+(token.get('showplayers_bar1') ? 'off' : 'on')+' --token',(token.get('showplayers_bar1') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,showplayers_bar2='+(token.get('showplayers_bar2') ? 'off' : 'on')+' --token',(token.get('showplayers_bar2') ? '3' : '_'),'transparent','black')+'</div></td>'
                +'</tr><tr>'
                    +'<td><b>Edit</b></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,playersedit_bar3='+(token.get('playersedit_bar3') ? 'off' : 'on')+' --token',(token.get('playersedit_bar3') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,playersedit_bar1='+(token.get('playersedit_bar1') ? 'off' : 'on')+' --token',(token.get('playersedit_bar1') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,playersedit_bar2='+(token.get('playersedit_bar2') ? 'off' : 'on')+' --token',(token.get('playersedit_bar2') ? '3' : '_'),'transparent','black')+'</div></td>'
                +'</tr>'
            +'</table>'
            //Aura Settings
            ...
            +'<table style="width:100%;table-layout:fixed;overflow:hidden;word-wrap:break-all;">'
                ...
                +'</tr><tr>'
                    +'<td><b>Radius</b></td>'
                    +'<td>'+makeButton('!pfc --token,aura1_radius=?{Aura Radius, in page units} --token',!_.isEmpty(token.get('aura1_radius')) ? token.get('aura1_radius') : '_','transparent',!_.isEmpty(token.get('aura1_radius')) ? 'black':'transparent')+'</td>'
                    +'<td>'+makeButton('!pfc --token,aura2_radius=?{Aura Radius, in page units} --token',!_.isEmpty(token.get('aura2_radius')) ? token.get('aura2_radius') : '_','transparent',!_.isEmpty(token.get('aura2_radius')) ? 'black':'transparent')+'</td>'
                +'</tr><tr>'
                    +'<td><b>Color</b></td>'
                    +'<td>'+makeButton('!pfc --token,aura1_color=?{Hex Color} --token',token.get('aura1_color'),token.get('aura1_color'),token.get('aura1_color'))+'</td>'
                    +'<td>'+makeButton('!pfc --token,aura2_color=?{Hex Color} --token',token.get('aura2_color'),token.get('aura2_color'),token.get('aura2_color'))+'</td>'
                +'</tr><tr>'
                    +'<td><b>Square</b></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,aura1_square='+(token.get('aura1_square') ? 'off' : 'on')+' --token',(token.get('aura1_square') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,aura2_square='+(token.get('aura2_square') ? 'off' : 'on')+' --token',(token.get('aura2_square') ? '3' : '_'),'transparent','black')+'</div></td>'
                +'</tr><tr>'
                    +'<td><b>Visible</b></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,showplayers_aura1='+(token.get('showplayers_aura1') ? 'off' : 'on')+' --token',(token.get('showplayers_aura1') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,showplayers_aura2='+(token.get('showplayers_aura2') ? 'off' : 'on')+' --token',(token.get('showplayers_aura2') ? '3' : '_'),'transparent','black')+'</div></td>'
                +'</tr><tr>'
                    +'<td><b>Edit</b></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,playersedit_aura1='+(token.get('playersedit_aura1') ? 'off' : 'on')+' --token',(token.get('playersedit_aura1') ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'<td><div style="font-family:pictos;">'+makeButton('!pfc --token,playersedit_aura2='+(token.get('playersedit_aura2') ? 'off' : 'on')+' --token',(token.get('playersedit_aura2') ? '3' : '_'),'transparent','black')+'</div></td>'
                +'</tr>'
            +'</table>'
            //Tint Color
            +'<br><b>Tint Color</b><div style="float:right;">'+makeButton('!pfc --token,tint_color=?{Hex Color} --token',token.get('tint_color'),token.get('tint_color'),token.get('tint_color'))+'</div><div style="clear:both;"></div>'
            //Light Settings
            +'<br><div style="textalign:center;font-size:110%;font-weight:bold;">Lighting</div>'
            +'<br><b>Radius</b><div style="float:right;">'+makeButton('!pfc --token,light_radius=?{Bright Radius, in page units} --token',!_.isEmpty(token.get('light_radius')) ? token.get('light_radius') : '_','transparent',!_.isEmpty(token.get('light_radius')) ? 'black':'transparent')+'</div><div style="clear:both;">'
            +'<b>Dim Radius</b><div style="float:right;">'+makeButton('!pfc --token,light_dimradius=?{Dim Radius, in page units} --token',!_.isEmpty(token.get('light_dimradius')) ? token.get('light_dimradius') : '_','transparent',!_.isEmpty(token.get('light_dimradius')) ? 'black':'transparent')+'</div><div style="clear:both;">'
            +'<b>Light Angle</b><div style="float:right;">'+makeButton('!pfc --token,light_angle=?{Degrees to light} --token',!_.isEmpty(token.get('light_angle')) ? token.get('light_angle') : '360','transparent','black')+'</div><div style="clear:both;">'
            +'<b>Sight</b><div style="float:right;font-family:pictos;">'+makeButton('!pfc --token,light_hassight='+(token.get('light_hassight') ? 'off' : 'on')+' --token',(token.get('light_hassight') ? '3' : '_'),'transparent','black')+'</div><div style="clear:both;">'
            +'<b>Visible</b><div style="float:right;font-family:pictos;">'+makeButton('!pfc --token,light_otherplayers='+(token.get('light_otherplayers') ? 'off' : 'on')+' --token',(token.get('light_otherplayers') ? '3' : '_'),'transparent','black')+'</div><div style="clear:both;">'
            +'<b>Vision Angle</b><div style="float:right;">'+makeButton('!pfc --token,light_losangle=?{Can see what degrees} --token',!_.isEmpty(token.get('light_losangle')) ? token.get('light_losangle') : '360','transparent','black')+'</div><div style="clear:both;">'
            +'<b>Multiplier</b><div style="float:right;">'+makeButton('!pfc --token,light_multiplier=?{Multiply Light by} --token',!_.isEmpty(token.get('light_multiplier')) ? token.get('aura1_radius') : '1','transparent','black')+'</div><div style="clear:both;">'
            : '');
            msg+='</div>';
            sendChat('Pathfinder Companion',msg,null,{noarchive:true});
        }
    ...
    statblockHandler = async function(text,characters,who){
        ...
        parser = async () =>{
            ...
            if(usesSpells){
                await createAttrWithWorker('use_spells',iChar.id,attributes,'1');
                                ...
                                let levelWorker = async (sl)=>{
                                    try{
                                        ...
                                        if(level){
                                            ...
                                            _.each(spells,(s)=>{
                                                workedSpells.push(createSpell(iChar,classNum,s,sl,spellStart,prepared));
                                            });
                                            return Promise.all(workedSpells);
                                        }else{
                ...
                sendChat('Pathfinder Companion Statblock Parser','/w "'+who+'" <u><b>['+iChar.get('name')+'](https://journal.roll20.net/character/'+iChar.id+')</b></u> imported',null,{noarchive:true});
            }
            if(!_.isEmpty(text)){
                _.defer(parser);
            }else{
    ...
    tokenSetupConfig = function(menu){
        ...
        return (!menu || menu==='TAS') ? '<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'//markermsg div start
            +'<b>Automatically create token actions for macro menus:</b><br>'+(!menu ? '<span style="font-family:pictos;">'+makeButton('!pfc --config,menu=TAS','l','transparent','black')+'</span>' : '')+'<div style="float:right;">'+makeButton('!pfc --config,TAS='+(state.PFCompanion.TAS==='auto' ? 'manual' : 'auto')+' --config'+(state.PFCompanion.TAS==='auto' ? '' : ',menu=TAS'),(state.PFCompanion.TAS==='auto' ? 'AUTO' : 'MANUAL'),(state.PFCompanion.TAS==='auto' ? 'green' : 'red'),'black')+'</div><div style="clear: both"></div>'
            ...
            '<br><table style="width:100%;">'
                ...
                +_.map(toCheck,(tc)=>{
                    return ('<tr>'
                        ...
                        +'<td><div style="font-family:pictos;">'+makeButton('!pfc --config,'+tc+'='+(state.PFCompanion.toCreate[tc]==='on' ? 'off' : 'on')+' --config,menu=TAS',(state.PFCompanion.toCreate[tc]==='on' ? '3' : '_'),'transparent','black')+'</div></td>'
                        +'<td><div style="font-family:pictos;">'+makeButton('!pfc --config,npc'+tc+'='+(state.PFCompanion.npcToCreate[tc]==='on' ? 'off' : 'on')+' --config,menu=TAS',(state.PFCompanion.npcToCreate[tc]==='on' ? '3' : '_'),'transparent','black')+'</div></td>'
                    +'</tr>')
                }).join('')
            +'</table><br>'+makeButton('!pfc --config','MAIN<br>MENU','transparent','black')+'<div style="float:right;">'+makeButton('!pfc --TAS,remove|ALL --config,'+_.map(toCheck,(tc)=>{return tc+'=off,npc'+tc+'=off'}).join(',')+' --config,menu=TAS','<u>RESET</u><br>REMOVE','#C0C0C0','#FF0000')+'</div>'
            :
            '')+'</div>' : '';
    },
    ...
    markerConfig = function(menu){
        var conditions = ['Blinded','Entangled','Invisible','Cowering','Fear','Pinned','Dazzled','Flat-Footed','Prone','Deafened','Grappled','Sickened','Helpless','Stunned','Fatigued','Exhausted','Energy Drain'],
            statusQuery='|';
        ...
        return (menu==='marker' || !menu) ? '<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'//markermsg div start
                +'<b>Apply Condition/Buff Statusmarkers:</b><br>'+(!menu ? '<span style="font-family:pictos;">'+makeButton('!pfc --config,menu=marker','l','transparent','black')+'</span>' : '')+'<div style="float:right;">'+makeButton('!pfc --config,markers='+(state.PFCompanion.markers.markers==='on' ? 'off' : 'on')+' --config'+(state.PFCompanion.markers.markers==='on' ? '' : 'menu=marker'),(state.PFCompanion.markers.markers==='on' ? 'ON' : 'OFF'),(state.PFCompanion.markers.markers==='on' ? 'green' : 'red'),'black')+'</div><div style="clear: both"></div>'
    ...
    tokenConfig = function(menu){
        ...
        return !menu ? '<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'//markermsg div start
                ...
                '<table style="width:100%;table-layout:fixed;overflow:hidden;word-break:break-all;">'
                    ...
                    +'</tr><tr>'
                        +'<td><b>Visible</b></td>'
                        +'<td><span style="font-family:pictos;">'+makeButton('!pfc --config,bar3Visible='+(state.PFCompanion.defaultToken.bar3Visible==='on' ? 'off' : 'on')+' --config',(state.PFCompanion.defaultToken.bar3Visible==='on' ? '3' : '_'),'transparent','black')+'</span></td>'
                        +'<td><span style="font-family:pictos;">'+makeButton('!pfc --config,bar1Visible='+(state.PFCompanion.defaultToken.bar1Visible==='on' ? 'off' : 'on')+' --config',(state.PFCompanion.defaultToken.bar1Visible==='on' ? '3' : '_'),'transparent','black')+'</span></td>'
                        +'<td><span style="font-family:pictos;">'+makeButton('!pfc --config,bar2Visible='+(state.PFCompanion.defaultToken.bar2Visible==='on' ? 'off' : 'on')+' --config',(state.PFCompanion.defaultToken.bar2Visible==='on' ? '3' : '_'),'transparent','black')+'</span></td>'
                    +'</tr>'
                +'</table>'
                :
                '')
    ...
    configAssembler = function(who,menu){
        var menu = '/w "'+who+'" <div style="border: 1px solid black; background-color: white; padding: 3px 3px;">'//overall div for nice formatting of control panel
                    ...
                    +'</div>'+tokenSetupConfig(menu)+resourceConfig(menu)+hpConfig(menu)+tokenConfig(menu)+markerConfig(menu)
                    +'<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'
                    +makeButton('!pfc --i','APPLY SETTINGS TO CAMPAIGN','#C0C0C0','#228B22')+'</div></div>';//end Control Panel Header div
	...
	cmdExtract = function(cmd){
        ...
        cmdSep.action = details.match(/^(?:config|help|TAS|resource|rest|whisper|apply|parse|grouproll|buffstatus|i$|token)/);
        if(cmdSep.action){
            cmdSep.action = cmdSep.action[0];
        }
        details=details.replace(cmdSep.action,'').replace(/^,/,'');
                ...
                if(obj.get('name')==='is_npc'){
                    ...
                    if(!_.isEmpty(character.get('controlledby'))||barLinked){
                        ...
                        if(cbName = statusquery[_.indexOf(statusquery,cbName)]){
                            applyStatus(character,cbName,parseInt(obj.get('current'))===0 ? false : (condName === 'Drained' ? Math.abs(parseInt(obj.get('current'))) : true));
                        }
                    }
                }
                break;
    ...
    campaignHandler = async function(obj,event,prev,depth){
        ...
        var oTurn=obj.get('turnorder'),
            ...
            buff,condition,character,name,removed,defaultToken,barLinked;
                                ...
                                if(obj.get('statusmarkers')!==prev.statusmarkers){
                                    ...
                                    _.each(removed,(r)=>{
                                        _.each(conditionMarkers,(c)=>{
                                            if(r.match(c.marker)){
                                                updated=true;
                                                if(r.match(/@\d/) && c.name==='Energy Drain'){
                                    ...
                                    _.each(added,(a)=>{
                                        _.each(conditionMarkers,(c)=>{
                                            if(a.match(c.marker)){
                                                updated=true;
                                                if(a.match(/@\d/) && c.name==='Energy Drain'){
