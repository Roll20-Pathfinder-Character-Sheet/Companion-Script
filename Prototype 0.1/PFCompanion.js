...
var PFCompanion = PFCompanion || (function() {
    ...
    checkInstall = function(){
        _.each(findObjs({type:'attribute',name:'pfsheet_version'},{caseInsensitive:true}),(a)=>{
            if(currSheet<a.get('current')*1){
                currSheet = a.get('current')*1;
            }
        });
    ...
    statusControl = function(character,who){
        ...
        buffs = _.filter(attributes,(a)=>{return a.get('name').match(/buff-name/)});
        conditions = _.filter(attributes,(a)=>{return a.get('name').match(/condition-/)});
        msg+='<b>Buffs:</b><br>';
        _.each(buffs,(b)=>{
    ...
    applyConditions = async function(character,condition,buff,swap,remove,rounds){
        ...
        if(buff){
            ...
            buffAttr=getAttrByName(cid,buffNameAttr.get('name').replace('buff-name','buff-enable_toggle'));
            buffState = (swap ? swapper[buffAttr+''] : (remove ? '0' : '1'));
            if(character.get('type')==='character'){
                buffNameAttr ? createAttrWithWorker(buffNameAttr.get('name').replace('buff-name','buff-enable_toggle'),cid,attributes,buffState) : undefined;
            }
        ...
        _.each(characters,(c)=>{
            if(rollName==='initiative'){
                ...
                if(existing!==null){
                    ...
                    turnTracker.push({'pr':(roll+mod+(mod/100)),'id':c.id});
                }
    ...
    configAssembler = function(who,menu){
        var menu = '/w "'+who+'" <div style="border: 1px solid black; background-color: white; padding: 3px 3px;">'//overall div for nice formatting of control panel
                    ...
                    +'</div>'+tokenSetupConfig(menu)+resourceConfig(menu)+hpConfig(menu)+mookConfig(menu)+tokenConfig(menu)+markerConfig(menu)
                    +'<div style="border-top: 1px solid #000000; border-radius: .2em; background-color: white;">'
    ...
    HandleInput = async function(msg_orig) {
		...
		args.shift();
		args.length===0 ? showHelp(who) : _.each(args,async (a)=>{
            ...
            switch(cmdDetails.action){
                ...
                case 'grouproll':
                    ...
                    if(cmdDetails.details.roll && playerIsGM(msg.playerid)){
                        ...
                        sendGroupRoll(tokens || characters,cmdDetails.details.roll,cmdDetails.details.whisper);
                    }
                    break;
                case 'parse':
                    if(!playerIsGM(msg.playerid)){
    ...
    attributeHandler = async function(obj,event,prev){
        try{
        if(obj.get('name').toLowerCase()==='pfsheet_version'){
            if(currSheet<obj.get('current')*1){
                currSheet = obj.get('current')*1;
                if(_.indexOf(sheetVersion,currSheet)!==-1){
                    sheetCompat=true;
                }else{
                    sheetCompat=false;
