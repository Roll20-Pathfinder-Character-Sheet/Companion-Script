...
*/
    ...
    var version = 'Prototype 0.172',
        sheetVersion = [1.62,1.63],
        lastUpdate = 1498256259,
        schemaVersion = 0.182,
        defaults = {
            css: {
                button: {
    ...
    affectWithSpellDialog = function(msg,who){
        ...
        if(!_.isEmpty(effectsMatch)){
            saveEffects = _.reject(effectsMatch,(e)=>{
                return saveMatch.match(new RegExp(e.replace(/\s*\d+\s+(?:minutes|rounds)/,''),'i')) || saveMatch.match(/negates?\s*$/i);
            });
        }else{
            saveEffects = [];
        }
        ...
        if(targetMatch){
            ...
            if(subTargets.area){
                ...
                let shape = subTargets.area.match(/centered on|within/) ? [(subTargets.area.match(/cube/) ? 'cube' : 'burst')+' centered on'] : subTargets.area.match(/(?:spread|cube|line|cone|burst|emanation)(?:\s+centered on)?/i);
                if(size && shape){
    ...
    mapBurst = async function(start,radius,center){
        try{
            var accumulator = [],
                path=[],
                initCoord = ['M',start.x+radius,start.y+(center ? 105 : 0)],
            ...
            center ? path.push(['L',path[0][1],path[0][2]]) : undefined;
            return path;
        }catch(err){
            sendError(err);
        }
    ...
    generateAOE = async function(tokens,size,shape,playerid){
        try{
            var page = getObj('page',tokens[0].get('pageid')),
                ...
                origin = tokens[0],
                target = tokens[1],
                center = shape.match(/centered on/i) ? true : false,
            ...
            shape = shape.replace(/\s*centered on\s*/,'');
            ...
            switch(shape){
                ...
                case 'cone':
                    ...
                    break;
                case 'burst':
                    path = await mapBurst({x:70*size/2+(center ? 35 : 0),y:70*size/2+(center ? 35 : 0)}, 70*size/2+(center ? 35 : 0),center);
                    width = size*70+(center ? 70 : 0);
                    height = size*70+(center ? 70 : 0);
                    break;
                case 'line'||'wall':
                ...
                case 'cube':
                    path = [
                        ['M',0,0],
                        ['L',0,70*size+(center ? 70 : 0)],
                        ['L',70*size+(center ? 70 : 0),70*size+(center ? 70 : 0)],
                        ['L',70*size+(center ? 70 : 0),0],
                        ['L',0,0]
                    ];
                    width = 70*size+(center ? 70 : 0);
                    height = 70*size+(center ? 70 : 0);
                    break;
            }
            if(!path){
                return;
            ...
            let newO = createObj('path',{
                ...
                top:origin.get('top')*1 + (center ? 0 : (shape.startsWith('cone') ? topMod*(height/2+ 35)+(height>size*70 ? 35 : 0) :35)),
                left:origin.get('left')*1+ (center ? 0 : (shape.startsWith('cone') ? leftMod*(width/2+ 35)+(width>size*70 ? 35 : 0) :35)),
                width:width,
                height:height,
            });
        }catch(err){
        ...
        _.each(detailKeys,(dk)=>{
            ...
            if(effectMatch){
                ...
                if(effectMatch[2]==='effects'){
                    ...
                    _.each(effectMatch[1].split(/\s*~\s*/),(e)=>{
    ...
    applyEffect = async function(token,details){
        ...
        var character = getObj('character',token.get('represents')),
            detailKeys = _.chain(details) .keys() .reject((k)=>{return _.isUndefined(details[k])}) .value(),
            effectTotals,
            mathop=0,
        ...
        if(effectTotals['effects']){
            let effectWorker = async () =>{
                try{
                    ...
                    let round = effect.match(/\d+\s+rounds?/) ? effect.match(/(\d+)\s+rounds?/)[1] : undefined;
                    effect = effect.replace(/\s*\d+\s+(?:rounds?|minutes?)/,'').trim();
                    condition = _.find(conditionList,(cl)=>{return cl.toLowerCase().match(effect)});
                    if(!condition){
                        buff = _.find(findObjs({type:'attribute',characterid:actor.id}),(a)=>{return a.get('name').match(/repeating_buff2_-[^_]+_name/) && a.get('current').match(effect)});
                        if(buff){
                            ...
                            await copyBuff(actor,character,buff.get('name').match(/repeating_buff2_(-[^_]+)_name/)[1],(newBuff ? newBuff.get('name').match(/repeating_buff2_(-[^_]+)_name/)[1] : null),mook);
                            buff = effect
                        }
                    }
                    if(condition || buff){
                        await applyConditions((mook ? token : character),condition,buff,null,null,round);
                    }
    ...
    copyBuff = async function(actor,target,rowID,currID,mook){
        try{
            var actorAttributes = findObjs({type:'attribute',characterid:actor.id}),
                fields = _.filter(actorAttributes,(a)=>{return a.get('name').match(rowID) && !a.get('name').match(/_b[1-6]_val$|_b[1-6]_formula$/)}),
                formulas = _.filter(actorAttributes,(aa)=>{return aa.get('name').match(/_b[1-6]_formula$/)}),
                newID = currID || generateRowID(),
                nFields = _.filter(findObjs({type:'attribute',characterid:target.id}),(a)=>{return a.get('name').match(newID)}),
                value,val=0,formulaAttr,
                workerAttr = ['-show','macro-text']
                fieldWorker = async () =>{
                    let f = fields.shift();
                    value = f.get('current');
                    if(f.get('name').match(/enable_toggle/)){
                        value = 0;
                    }
                    if(f.get('name').match(/repeating_buff2_-[^_]+_b[1-6]_macro-text/)){
                        formulaAttr = _.find(formulas,(form)=>{return form.get('name')===f.get('name').replace(/macro-text/,'formula')}) || createObj('attribute',{
                                name: f.get('name').replace(/macro-text/,'formula'),
                                current:f.get('current'),
                                characterid:actor.id
                            });
                        value = formulaAttr.get('current');
                        _.each((value+'').match(/\d+d\d+|@{.*?(?=})}/g),(v)=>{
                            let dice = v.match(/(\d+)d(\d+)/);
                            let attributeMatch = v.match(/@{(?:(.*?(?=\|))\|)?(.*?(?=\||}))(?:\|(max))?}/);
                            if(dice){
                                _.each(_.range(dice[1]),(r)=>{val+=randomInteger(dice[2])});
                            }else if(attributeMatch){
                                val = getAttrByName((attributeMatch[1] ? findObjs({type:'character',name:attributeMatch[1]},{caseInsensitive:true})[0] : actor.id),attributeMatch[2],(attributeMatch[3] ? 'max':null));
                            }
                            value = value.replace(v,val);
                        });
                    }
                    let nBuff = await createAttrWithWorker(f.get('name').replace(/_-[^_]+_/,'_'+newID+'_'),target.id,nFields,value,null,(_.some(workerAttr,(wa)=>{return f.get('name').match(wa)}) ? false : true));
                    if(!_.isEmpty(fields)){
                        return fieldWorker();
                    }else{
