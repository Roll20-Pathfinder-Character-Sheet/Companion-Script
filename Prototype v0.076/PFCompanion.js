...
var PFCompanion = PFCompanion || (function() {
    ...
    parseStatblock = function(block){
        var defenseMatch=/DEFENSE\n+(AC\s+[^\n]+)\n+hp\s+([^\n]+)\n+Fort\s+([^,]+),\s+Ref\s+([^,]+),\s+Will\s+([^\n]+)\n+(?:Defensive\s+Abilities\s+([^;\n]+))?[;\n]?(?:\s+DR\s+([^;]+);)?(?:\s+)?(?:Immune\s([^;]+);)?(?:\s+)?(?:Resist\s+([^;]+);)?(?:\s+)?(?:SR\s+(\d+))?(?:\n+)?/,
            offenseMatch=/OFFENSE\n+Speed\s+([^\n]+)\n+(?:Melee\s+([^\n]+)\n+)?(?:Ranged\s+([^\n]+)(?:\n+)?)?(?:Space\s+([^;]+);)?(?:\s+Reach\s+([^\n]+)(?:\n+)?)?(?:Special Attacks\s+([^\n]+))?(?:\n+)?(?:((?:Spell-Like Abilities|Spells Prepared|Spells Known)\s+.+\n+(?:.+\n+)+))?/,
            ...
            defaultMatch=/(?:Avatar\s+([^\n]+)\n+)?(?:Token\s+([^\n]+)\n+)?([^\n]+)\n+([\w\s]+)(?:\s+)?CR\s+([^\n]+)\n+XP\s+([^\n]+)\n+(?:([\w\s]+\d+)\n+)?(LG|NG|CG|NG|N|CN|LE|NE|CE)\s+(Fine|Diminutive|Tiny|Small|Medium|Large|Huge|Gargantuan|Colossal)\s+([\w\s]+)(?:\s+(\([\w\s,]+\)))?\n+Init\s+([^;]+);\s+Senses\s+([^;]+;\s+Perception\s+[^\n]+)\n+(?:Weakness\s+([^\n]+)\n+)?(?:Aura\s+([^\n]+)\n+)?/,
            matched;
            
        switch(true){
            case block.indexOf('DEFENSE\n')===0:
                matched=block.match(defenseMatch);
                break;
            case block.indexOf('OFFENSE')===0:
                matched=block.match(offenseMatch);
                break;
            case block.indexOf('TACTICS')===0:
                matched=block.match(tacticsMatch);
                break;
            case block.indexOf('STATISTICS')===0:
                matched=block.match(statisticsMatch);
                break;
            case block.indexOf('SPECIAL ABILITIES')===0:
                matched=block.match(saMatch);
                break;
            case block.indexOf('ECOLOGY')===0:
                matched=block.match(ecologyMatch);
                break;
            default:
                matched=block.match(defaultMatch);
                break;
        }
        return matched;
    },
    ...
    statblockHandler = async function(text,characters){
        var block,iChar,blockSection,attributes,
            convert = {
                ...
                'statistics':[null,'str_compendium','dex_compendium','con_compendium','int_compendium','wis_compendium','cha_compendium','bab_compendium','cmb_compendium','cmd_compendium','npc-feats-text','skills_compendium','languages','SQ_compendium'],
                'tactics':[null,'tactics'],
                'ecology':[null,'ecology'],
                'special':[null,'content_compendium'],
            ...
            ecologyMatch=/((?:ECOLOGY)\n+(?:.*(?:\n+)?)+)/,
            defaultMatch=/(?:Avatar\s+([^\n]+)\n+)?(?:Token\s+([^\n]+)\n+)?([^\n]+)\n+([\w\s]+)(?:\s+)?CR\s+([^\n]+)\n+XP\s+([^\n]+)\n+(?:([\w\s]+\d+)\n+)?(LG|NG|CG|NG|N|CN|LE|NE|CE)\s+(Fine|Diminutive|Tiny|Small|Medium|Large|Huge|Gargantuan|Colossal)\s+([\w\s]+)(?:\s+(\([\w\s,]+\)))?\n+Init\s+([^;]+);\s+Senses\s+([^;]+;\s+Perception\s+[^\n]+)\n+(?:Weakness\s+([^\n]+)\n+)?(?:Aura\s+([^\n]+)\n+)?/,
            accrue,attributesToSet,description,setAttr,
            charList=[],
            section,keys,parser,parseSection,statBlock,
            start = _.now();
        text =_.chain(text.replace(/<br\/>/g,'').replace(/<br>/g,'\n').split(/\nNEW CREATURE\n/))
            .map((t)=>{
                return t.replace(/\ndefense\n/gi,'___STATBLOCK PARSE SITE___ DEFENSE\n').replace(/\noffense\n/gi,'___STATBLOCK PARSE SITE___ OFFENSE\n').replace(/\ntactics\n/gi,'___STATBLOCK PARSE SITE___ TACTICS\n').replace(/\nstatistics\n/gi,'___STATBLOCK PARSE SITE___ STATISTICS\n').replace(/\nspecial abilitie\n/gi,'___STATBLOCK PARSE SITE___ SPECIAL ABILITIES\n').replace(/\necology\n/gi,'___STATBLOCK PARSE SITE___ ECOLOGY\n').split('___STATBLOCK PARSE SITE___ ');
            })
            .reject((t)=>{return(_.isUndefined(t) || _.isEmpty(t))})
            .value();
        /*text = [[["This barbaric humanoid bears ragged equipment and armor in sullen colors. It has coarse body hair and a stooped posture like some primitive man but with a grayish-green skin tone and bestial facial features beneath a black hood. Burning red eyes peer below a low, sloping brow, just above a flattened nose, and prominent tusk-like teeth.\n\nOrc\tCR 1/3\nXP 135\nOrc warrior 1\nCE Medium humanoid\nInit +0; Senses darkvision 60 ft.; Perception –1\nWeakness light sensitivity\n",null,null,"This barbaric humanoid bears ragged equipment and armor in sullen colors. It has coarse body hair and a stooped posture like some primitive man but with a grayish-green skin tone and bestial facial features beneath a black hood. Burning red eyes peer below a low, sloping brow, just above a flattened nose, and prominent tusk-like teeth.","Orc\t","1/3","135","Orc warrior 1","CE","Medium","humanoid",null,"+0","darkvision 60 ft.; Perception –1","light sensitivity",null],
                                    ["DEFENSE\n\nAC 13, touch 10, flat-footed 13 (+3 armor)\nhp 6 (1d10+1)\nFort +3, Ref +0, Will –1\nDefensive Abilities ferocity\n","AC 13, touch 10, flat-footed 13 (+3 armor)","6 (1d10+1)","+3","+0","–1","ferocity",null,null,null,null],
                                    ["OFFENSE\n\nSpeed 30 ft.\nMelee falchion +5 (2d4+4/18–20)\nRanged javelin +1 (1d6+3)\n","30 ft.","falchion +5 (2d4+4/18–20)","javelin +1 (1d6+3)",null,null,null,null],
        ...
        parser = () =>{
            ...
            createAttrWithWorker('config-show',iChar.id,attributes,'0');
            ...
            parseSection = () =>{
                ...
                if(block){
                    section = block[0].match(/^(defense|offense|statistics|tactics|ecology|special)/i);
                    section = section ? section[0].toLowerCase() : 'default';
                ...
                if(!_.isEmpty(statBlock)){
                    ...
                    _.each(keys,(k)=>{
                        switch(true){
                            case k==='ecology':
                                description = (description ? description+'<br>' : '')+accrue[k].replace(/\n/g,'<br>')+'<br>';
                                break;
                            case k==='avatar':
                            ...
                            case k==='token':
                                ...
                                break;
                            case k==='description':
                                description=accrue[k].replace(/\n/g,'<br>')+(description ? '<br>'+description : '');
                                break;
                            case k==='character_name':
                                iChar.set('name',accrue[k]);
                                break;
                            case k==='class':
                            ...
                            case k==='init_compendium':
                                ...
                                break;
                            case k==='tactics':
                                if(description.match(/<br>TACTICS<br>/)){
                                    description = description.replace('ECOLOGY',accrue[k].replace(/\n/g,'<br>')+'<br>ECOLOGY');
                                }else{
                                    description = (description ? description+'<br>' : '')+accrue[k].replace(/\n/g,'<br>')+'<br>';
                                }
                                break;
                            default:
                                attributesToSet[k] = accrue[k].replace(/\n/g,'<br>');
                                break;
                        }
                    });
                    iChar.set('gmnotes',(description ? description : ''));
                    ...
                    createAttrWithWorker('npc_import_now',iChar.id,attributes,'1');
                    log('  > Pathfinder Companion Statblock Parser:'+iChar.get('name')+' imported <');
                    if(!_.isEmpty(text)){
                        _.defer(parser);
                    }else{
                        log('  > Pathfinder Companion Statblock Parser: All NPCs parsed and imported in '+((_.now()-start)/1000)+' seconds <');
                    }
                }
            };
            _.defer(parseSection);
    ...
    setParseAttr = function(accrue,iChar,attributes){
        var keys = _.reject(_.keys(accrue),(k)=>{return (_.isUndefined(k) || _.isEmpty(k))}),
            setAttr;
        _.each(keys,(k)=>{
            if(k.toLowerCase()==='ecology'){
                log(k);
            }
            if(accrue[k] && k){
                ...
                switch(true){
                    ...
                    case k==='token':
                        ...
                        //defaulttoken setup
                    case k==='description':
                        iChar.set('gmnotes',accrue[k]);
                        break;
                    case k==='character_name':
                        iChar.set('name',accrue[k]);
                        break;
                    case k==='class':
                        setAttr = _.find(attributes,(a)=>{return a.get('name')==='init_compendium'});
                        if(setAttr){
                            setAttr.set('current',accrue[k]+' '+setAttr.get('current').trim())
                        }else{
                            setAttr = createObj('attribute',{characterid:iChar.id,name:'init_compendium',current:accrue[k],max:''});
                            attributes.push(setAttr);
                        }
                        break;
                    case k==='subtype':
                        setAttr = _.find(findObjs({type:'attribute',characterid:iChar.id}),(a)=>{return a.get('name')==='type_compendium'});
                        if(setAttr){
                            setAttr.set('current',setAttr.get('current')+'/'+accrue[k])
                        }else{
                            setAttr = createObj('attribute',{characterid:iChar.id,name:'type_compendium',current:'/'+accrue[k],max:''});
                            attributes.push(setAttr);
                        }
                        break;
                    case k==='init_compendium':
                        setAttr = _.find(findObjs({type:'attribute',characterid:iChar.id}),(a)=>{return a.get('name')==='init_compendium'});
                        if(setAttr){
                            setAttr.set('current',setAttr.get('current').trim()+' '+accrue[k])
                        }else{
                            setAttr = createObj('attribute',{characterid:iChar.id,name:k,current:accrue[k],max:''});
                            attributes.push(setAttr);
                        }
                        break;
                    case k==='tactics':
                        iChar.get('gmnotes',(n)=>{
                            iChar.set('gmnotes',((n && n!=='null') ? n+'<br>TACTICS<br>'+accrue[k] : 'TACTICS<br>'+accrue[k]));
                        });
                        break;
                    default:
                        setAttr = _.find(findObjs({type:'attribute',characterid:iChar.id}),(a)=>{return a.get('name')===k});
                        if(setAttr){
                            setAttr.set('current',accrue[k]);
                        }else{
                            setAttr = createObj('attribute',{characterid:iChar.id,name:k,current:accrue[k],max:''});
                            attributes.push(setAttr);
                        }
                ...
                log('  > Pathfinder Companion Statblock Parser:'+k+' parsed <');
            }
        });
    },
